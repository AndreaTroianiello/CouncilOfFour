...
public class RankPanel extends JPanel {
	...
	private void init() {
		...
		lim.gridwidth = 3;
		layout.setConstraints(rankLabel, lim);// applicazione del layout alla
												// label
		add(rankLabel);// aggiunta della label al panel

		// ----------tabella finta----------
		JLabel spaceLabel1 = new JLabel(addSpace(lung / 100));// aggiunge spazi
																// per centrare
																// la tabella
		lim.gridx = 0;// posizione componenti nella griglia
		lim.gridy = 1;
		...
		lim.fill = GridBagConstraints.NONE;// grandezza componenti nei riquadri
											// (both= tutto pieno)
		layout.setConstraints(spaceLabel1, lim);// applicazione del layout alla
												// label
		...
		lim.fill = GridBagConstraints.BOTH;// grandezza componenti nei riquadri
											// (both= tutto pieno)
		JScrollPane scrollTable = new JScrollPane(rank);// applicazione dello
														// scroll al panel
		...
		// ----------tabella finta----------
		JLabel spaceLabel2 = new JLabel(addSpace(lung / 100));// aggiunge spazi
																// per centrare
																// la tabella
		...
		lim.fill = GridBagConstraints.NONE;// grandezza componenti nei riquadri
											// (both= tutto pieno)
		layout.setConstraints(spaceLabel2, lim);// applicazione del layout alla
												// label
		add(spaceLabel2);// aggiunta della label al panel

		// ----------button panel (sell + buy)----------
		JPanel littlePanel = new JPanel();// nuovo pannello per i bottoni buy
											// and sell
		...
		lim.fill = GridBagConstraints.NONE;// grandezza componenti nei riquadri
											// (both= tutto pieno)
		layout.setConstraints(littlePanel, lim);// applicazione del layout al
												// littlePanel
	...
	private void initRankTable() {
		model = new DefaultTableModel(new Object[][] {}, new String[] { "Position", "Username", "#Emporiums built",
				"Assistants", "Richness", "#Business Permit Tiles", "Hand", "Nobility Track", "Victory Track" }) {
	...
	public void fillRankTable(List<Player> players) {
		...
		for (int index = 0; index < players.size(); index++) {
			...
			model.addRow(new Object[] { index + 1, player.getUser(), player.getEmporiums().size(),
					player.getAssistantsPool().getAssistants(), player.getRichness().getCoins(),
					player.getAvailableBusinessPermits().size() + player.getUsedBusinessPermit().size(),
					player.getHand().size(), player.getNobilityBoxPosition(),

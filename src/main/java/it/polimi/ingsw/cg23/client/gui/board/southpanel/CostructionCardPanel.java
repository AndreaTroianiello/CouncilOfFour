...
public class CostructionCardPanel {
	 ...
	 */
	public JPanel getShowCostructionCard(Region reg) {// carte costruzioni
														// mostrate delle
														// regioni
		...
		// ----------carte costruzione----------
		BufferedImage img1 = getCostructionImg(reg.getName());// carta
																// costruzione 1
		JLabel defaultCostruction = new JLabel(new ImageIcon(img1));// aggiungo
																	// l'immagine
																	// alla
																	// label
		defaultCostruction.setToolTipText("Mazzo carte costruzione " + reg.getName());
		defaultCostruction.setBounds(0, 0, img1.getWidth(), img1.getHeight());// dimensioni
																				// della
																				// label
		...
		lim.gridwidth = 1;
		layout.setConstraints(defaultCostruction, lim);// applicazione del
														// layout al label
		...
		for (int i = 0; i < bpt.size(); i++) {// scorre le carte costruzione
												// visibili

			// ----------carte costruzione----------
			BufferedImage img = getCostructionImg(nameCostructor(bpt.get(i).getCitiesId()));// carta
																							// costruzione
																							// 1
			JLabel costructionCard = new JLabel(new ImageIcon(img));// aggiungo
																	// l'immagine
																	// alla
																	// label
			...
			costructionCard.setBounds(0, 0, img.getWidth(), img.getHeight());// dimensioni
																				// della
																				// label
			...
			lim.gridheight = 1;// grandezza del riquadro
			lim.gridwidth = 1;
			layout.setConstraints(costructionCard, lim);// applicazione del
														// layout al label
			final int k = i;
			...
			costructionCard.addMouseListener(new MouseListener() {
				...
				@Override
				public void mouseClicked(MouseEvent e) {// azioni al click del
														// mouse
					if (SwingUtilities.isLeftMouseButton(e)) {// bottone
																// sinistro
					...
					if (SwingUtilities.isRightMouseButton(e)) // bottone destro
						writeArea(bpt.get(k));// scrittura delle informazioni
												// della carta costruzione sulla
												// logger area
				}
			});
	 ...
	 */
	public JPanel myCostructionCard(Player p) {// carte costruzione usabili dal
												// giocatore
		...
		GridBagConstraints lim = new GridBagConstraints();// definizione del
															// layout
		lim.fill = GridBagConstraints.NONE;// grandezza componenti nei riquadri
											// (both= tutto pieno)
		lim.anchor = GridBagConstraints.WEST;// posizione componenti nei
												// riquadri

		int q = 0;// posizione griglia
		List<BusinessPermitTile> bpt = p.getAvailableBusinessPermits();// carte
																		// costruzione
																		// disponibili
		...
		for (int i = 0; i < bpt.size(); i++) {// scorre le carte costruzione
			...
			final int k = i;
			costructionCard.addMouseListener(new MouseListener() {// azioni
																	// pannello
																	// click
																	// mouse
				@Override
	 ...
	 */
	private BufferedImage getCostructionImg(String name) {// recupero l'immagine
															// della carta
															// costruzione
		BufferedImage image = null;
		String path = "src/main/resources/images/costruction/" + name + ".png";// percorso
																				// dell'immagine
	 ...
	 * 
	 * @param id,
	 *            the list of city id of a costruction card
	 * @return a string with the city id
	 */
	private String nameCostructor(List<Character> id) {
		String nome = "";

		for (int i = 0; i < id.size(); i++) {// scorro il numero di city id di
												// una carta costruzione
	...
	public void writeArea(BusinessPermitTile bpt) {
		loggerArea.append("\nCarta costruzione " + bpt.getZone());// aggiungo
																	// alla
																	// loggerArea
																	// le
																	// informazioni
		...
		for (int i = 0; i < bpt.getCitiesId().size(); i++) {// scorro gli id
															// citta' della
															// carta costrucione
	 ...
	 * 
	 * @param bpt,
	 *            the business permit tiles
	 * @return the jlabel image
	 */
	public JLabel oldCostructionWB(BusinessPermitTile bpt) {
		BufferedImage image = getCostructionImg(nameCostructor(bpt.getCitiesId()));// caricamento
																					// immagine

		ColorSpace cs = ColorSpace.getInstance(ColorSpace.CS_GRAY);
		ColorConvertOp ccop = new ColorConvertOp(cs, null);

		BufferedImage imgWB = ccop.filter(image, null);// trasformazione
														// immagine in bianco e
														// nero

		return new JLabel(new ImageIcon(imgWB));// ritorno la label con la carta
												// costruzione
	}
}

...
public class PoliticCardPanel {
	...
	public JPanel createCard() {
		...
		GridBagConstraints lim = new GridBagConstraints(); // impostazioni
															// layout
		lim.fill = GridBagConstraints.NONE;// grandezza componenti nei riquadri
											// (both= tutto pieno)
		lim.anchor = GridBagConstraints.EAST;// posizione componenti nei
												// riquadri

		for (int i = 0; i < p.getHand().size(); i++) {// scorre le carte
														// politiche
			// ----------carta politica----------
			BufferedImage img;
			String color;
			...
			if (card.isJolly()) {// carta politica jolly
				...
				color = new ColorManager().getColorName(card.getColor());// recupero
																			// il
																			// colore
																			// della
																			// carta
			...
			JButton politicButton = new JButton(new ImageIcon(img));// bottone
																	// carta
																	// politica
			...
			layout.setConstraints(politicButton, lim);// applicazione del layout
														// al bottone della
														// carta politica
			panel.add(politicButton);// aggiunta della carta politica al
										// pannello
			politicButton.addMouseListener(new MouseListener() {// azioni del
																// mouse per la
																// carta
																// politica
				@Override
	 ...
	 */
	private BufferedImage politcsImg(String name) {// recupero l'immagine delle
													// carte politiche
		BufferedImage image = null;
		String path = "src/main/resources/images/politics/" + name + ".png";// percorso
																			// dell'immagine

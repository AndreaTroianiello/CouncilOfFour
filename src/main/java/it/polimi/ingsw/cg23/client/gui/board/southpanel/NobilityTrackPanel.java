...
public class NobilityTrackPanel extends JPanel {
	...
	public NobilityTrackPanel(ClientModel model, JTextArea loggerArea) {
		...
		widthBox = (9.0 / 16) * lung / nt.getNobilityBoxes().size();// larghezza
																	// del
																	// nobility
																	// box
	...
	private void init() {
		...
		for (int i = 0; i < nt.getNobilityBoxes().size(); i++) {// ciclo che
																// scorre il
																// numero di
																// nobility box

			BufferedImage img = nobilityImg(i);// carica l'immagine del nobility
												// box
			double height = ((double) img.getHeight() / img.getWidth()) * widthBox;
			Image myim = img.getScaledInstance((int) widthBox, (int) height, Image.SCALE_DEFAULT);// ridimensionamento
																									// immagine

			JLabel boxLabel = new JLabel(new ImageIcon(myim));// creazione della
																// label con
																// l'immagine
																// del nobility
																// box
			...
			layout.setConstraints(boxLabel, lim);// applicazione del layout alla
													// label
			add(boxLabel);// aggiunta della label al panello

			final int k = i;
			boxLabel.addMouseListener(new MouseListener() {// azioni al click
															// del mouse
				@Override
	...
	/**
	 * calcola the le lenght of nobility track @return, the lenght of nobility
	 * track
	 */
	 ...
	 * 
	 * @param loggerArea,
	 *            the area to read the information
	 * @param nt,
	 *            the nobility track
	 * @param k,
	 *            the actual nobility box
	 */
	private void writeArea(JTextArea loggerArea, NobilityTrack nt, int k) {
		loggerArea.append("\nNobility Track " + k + "");

		if (nt.getNobilityBoxes().get(k).getBonus().toString() != "[]") {// bonus
																			// nullo
			loggerArea.append("\n  Bonus: ");
			String bonus = "";

			for (int j = 0; j < nt.getNobilityBoxes().get(k).getBonus().size(); j++) {// bonus
																						// del
																						// nobility
																						// box
		...
		if (nt.getNobilityBoxes().get(k).getPlayers().toString() != "[]") {// nessun
																			// player
			loggerArea.append("\n  Player: ");
			String player = "";

			for (int j = 0; j < nt.getNobilityBoxes().get(k).getPlayers().size(); j++) {// giocatori
																						// del
																						// nobility
																						// box
	 ...
	 * 
	 * @param num,
	 *            the number of nobility box
	 * @return the nobility box images
	 */
	private BufferedImage nobilityImg(int num) {// recupero l'immagine del
												// nobility box
		BufferedImage image = null;
		String path = "src/main/resources/images/nobilityTrack/" + num + ".png";// percorso
																				// dell'immagine

...
/**
 * The client Command Line Interface's controller. This manages the inputs of
 * the user.
 * 
 * @author Andrea
 *
...
public class ControllerCLI implements ClientController {
	 ...
	 * 
	 * @param out
	 *            The class that manages the objects to send at the server.
	 */
	@Override
	 ...
	 * 
	 * @param bonus
	 *            If the bonus is null the controller is in normal mode, else if
	 *            isn't null elaborate the bonus during the update.
	 */
	...
	public void updateController(Change change) {
		...
		if (change instanceof BonusChange) {
			cli.print("", "You have activated the " + ((BonusChange) change).getBonus().toString()

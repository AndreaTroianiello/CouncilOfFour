...
public class Turn {
	 ...
	 * 
	 * @return If true the game is finished and the first player of the list has
	 *         built all emporiums available.
	 * @throws NegativeNumberException
	 */
	public boolean changePlayer() throws NegativeNumberException {
		State status = board.getStatus();
		if (players.get((currentPlayer + 1) % players.size()) != status.getFinalPlayer()) { // Control
																							// if
																							// the
																							// next
																							// player
	 ...
	 */
	public boolean isFinalTurn() { // Return true if a player has built all
									// emporiums.
	...
	private void controlAction() {
		if (action.isMain() && mainAction) { // Control if the action is a main
												// action and it's authorized.
			--mainIndex; // Decrement the main action's counter.
			if (mainIndex == 0)
				mainAction = false;
		}
		if (!action.isMain() && secondAction) // Control if the action is not a
												// main action and it's
												// authorized.
			secondAction = false;
		if (getCurrentPlayer().isAdditionalAction()) { // Control if the second
														// action has enabled a
														// new main action.
			mainIndex++;
			mainAction = true;
			getCurrentPlayer().switchAdditionalAction();
	...
	private void runActionNormal() {
		...
		State status = board.getStatus();
		if ((action.isMain() && mainAction) || (!action.isMain() && secondAction)) { // Control
																						// if
																						// the
																						// action
																						// is
		...
		if (status.getFinalPlayer() == null && player.isAvailableEmporiumEmpty()) // If
																					// the
																					// current
																					// player
																					// has
	...
	private void runActionMarket() {
		...
		if (action instanceof MarketSell && "MARKET: SELLING".equals(status.getStatus())
				|| action instanceof MarketBuy && "MARKET: BUYING".equals(status.getStatus())) { // Control
																									// if
																									// the
																									// action
																									// permits
																									...
																									// a
																									// item.
			action.runAction(player, board); // Run the action. //Control the
												// authorization.
		} else
			action.notifyObserver(new InfoChange("Action refused."));
	...
	/**
	 * Controls if the action is a normal action or a market's action and runs
	 * it.
	 */
	...
	public String toString() {
		return "Turn [currentPlayer=" + getCurrentPlayer().getUser() + ", mainIndex=" + mainIndex + ", mainAction="
				+ mainAction + ", secondAction=" + secondAction + "]";
	}

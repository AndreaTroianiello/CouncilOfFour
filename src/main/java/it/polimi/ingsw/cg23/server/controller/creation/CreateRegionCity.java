...
public class CreateRegionCity {
	...
	public List<Region> createRegions(BonusKing bk) {
		...
		for (int i = 0; i < regionNumber; i++) {// ciclo che scorre le regioni
			...
			int regBonus = Integer.parseInt(regionBonus[i][1]);// trasformo i
																// bonus regione
																// in interi
			Region r = new Region(cityInfo[i * c][4], regBonus, rd, bk);// creo
																		// la
																		// regione
	 ...
	 */
	public List<City> createCities(int j, Region r, List<Type> typeList) {// crea
																			// le
																			// citta'
																			// della
																			// regione
		...
		for (int i = 0; i < cityInfo.length / regionNumber; i++) {// ciclo che
																	// scorre le
																	// citta' di
																	// una
																	// regione
			...
			for (int k = 0; k < typeList.size(); k++) {
				if (cityInfo[ii][1].equals(typeList.get(k).getName())) {
					City c = new City(cityInfo[ii][3].charAt(0), cityInfo[ii][0], typeList.get(k), r);// creo
																										// la
																										// citta'
	...
	public void addNeighbors(List<City> citta) {
		for (int h = 0; h < citta.size(); h++) {// scorre le citta' a cui
												// aggiungere i vicini
			for (int i = 0; i < cityInfo.length; i++) {// scorre le citta' prese
														// dall'xml
				if (cityInfo[i][0].equals(citta.get(h).getName())) {// cerco la
																	// citta'
																	// attuale
																	// da quelle
																	// dell'xml
	 ...
	 *            the xml city
	 * @param h
	 *            the city to add neighbors
	 */
	private void nearVicini(int i, int h, List<City> citta) {// rompe la
																// funzione
																// addNeighbors
																// che aveva
																// troppi
																// innesti
		for (int k = 0; k < cityInfo[i][2].length(); k++) {// scorro il numero
															// di link delle
															// citta'
			char link = cityInfo[i][2].charAt(k);
			for (int j = 0; j < citta.size(); j++) {// scorro le citta'
				if (link == citta.get(j).getId()) {
					citta.get(h).addNeighbor(citta.get(j));// aggiungo alla
															// citta' il vicino
															// trovato
				}
			}

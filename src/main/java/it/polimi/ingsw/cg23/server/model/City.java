...
/**
 * The city's class. The city is characterized by an id and a name. It contais a
 * list of nearly cities and a list of builded emporiums. The city has a region
 * and a type.
 * 
 * @author Andrea
 *
...
public class City implements Serializable {
	...
	private final String name; // The name of the city, the first char is the
								// id.
	...
	private final List<Emporium> emporiums; // The list of emporiums builded in
											// the city.
	 ...
	 * 
	 * @param id
	 *            The id of the city. It's the first character of the name.
	 * @param name
	 *            The name of the city.
	 * @param type
	 *            The type of the city.
	 * @param region
	 *            The region that contains the city.
	 */
	...
	public boolean containsEmporium(Player player) {
		...
		for (Emporium emporium : emporiums) { // Extract the emporium.
			Player emporiumPlayer = emporium.getPlayer(); // Get the player of
															// the emporium.
	...
	/**
	 * Builds an emporium in the city. When a player builds a emporium loses
	 * assistants. The assistants of the player can't be negative. This method
	 * doesn't control if the player hasn't a emporium in this city.
	 * 
	 * @param emporium
	 *            an emporium of the player.
	 * @throws NegativeNumberException
	 *             the assistants of the player must be positive.
	 */
	public void buildEmporium(Emporium emporium) throws NegativeNumberException {
		...
		player.getAssistantsPool().setAssistants(assistantsPlayer); // If sets a
																	// negative
																	// number
																	// throws a
		...
		runBonusCityAndNeighbors(player, new ArrayList<City>()); // Runs the
																	// bonus of
																	// the city
																	// and
																	// visits
	...
	/**
	 * Runs the bonus associated to the city (only these). The neighbors are not
	 * visited.
	 * 
	 * @param player
	 *            the player who is to receive the bonus.
	 */
	 ...
	 *            the player who is to receive the bonus.
	 * @param citiesVisited
	 *            the list of the cities already visited.
	 */
	public void runBonusCityAndNeighbors(Player player, List<City> citiesVisited) {
		boolean containsEmporium = containsEmporium(player); // Control if this
																// city contains
																// a player's
																// emporium.

		// Execute if the city is just not visited and doesn't contain a
		// player's emporium
		if (!citiesVisited.contains(this) && containsEmporium) {
			runBonusCity(player); // Run the bonus
			citiesVisited.add(this); // Add this city at the list of city at
										// citiesVisited
			for (City neighbor : neighbors) { // Visit the neighbors
				neighbor.runBonusCityAndNeighbors(player, citiesVisited); // Visit
																			// the
																			// neighbor
																			// and
																			// run
	 ...
	 * @param path
	 *            the list of cities already visited.
	...
	public double minimumDistance(City target, List<City> path) {
		if (this.equals(target)) // If this city is the target returns 0.
			...
			ArrayList<City> neighborsList = new ArrayList<>(); // Creates a new
																// list for the
																// neighbors.
			neighborsList.addAll(0, this.neighbors); // Copies the original list
														// of neighbors.
			neighborsList.removeAll(path); // Removes all cities already visited
											// from neighbors' list
			path.add(this); // Adds this city at the list of cities already
							// visited.
			double minDistance = Double.POSITIVE_INFINITY; // The minimum
															// default distance
															// has infinite
															// value.
			for (City neighbor : neighborsList) // Explores the neighbors' list.
			{
				double distanceThroughSource = neighbor.minimumDistance(target, path); // Gets
																						// the
																						// minimum
																						// distance
																						// of
																						// the
																						// neighbor.
				if (distanceThroughSource < minDistance) // Compares the
															// distance.
					minDistance = distanceThroughSource; // If the new distance
															// is smaller than
															// the old, this
															// becomes the new
															// minimum distance.
			}
			path.remove(this); // Removes this city from the path. It can be
								// visited again by new path.
			return minDistance + 1; // Returns the minimum distance increased by
									// one.
		}
	}
	...
	public String toString() {
		return "City [id=" + id + ", name=" + name + ", region=" + region.getName() + ", bonus=" + token.toString()
				+ ", type=" + type.getName() + ", neighbors=" + neighbors.size() + "]";
	}
}

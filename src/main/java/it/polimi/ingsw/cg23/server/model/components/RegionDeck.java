...
public class RegionDeck implements Serializable {
	...
	/**
	 * Returns the status of hidden and showed deck. If is true, all decks are
	 * empty.
	 * 
	...
	public void setBusinessPermitTiles(List<BusinessPermitTile> businessPermitTiles) {
		...
		for (int index = 0; index < maxTilesShowed; ++index) // Puts
																// maxTilesShowed
																// tiles in the
																// showed deck.
			showedDeck.add(businessPermitTiles.remove(0));
		while (!businessPermitTiles.isEmpty()) // Puts the other tiles in the
												// hidden deck.
	 ...
	 * 
	 * @param businessPermitTiles
	 *            the tiles that need to shuffle.
	 */
	private void shuffleTitle(List<BusinessPermitTile> businessPermitTiles) {
		...
		for (int index = businessPermitTiles.size(); index > 0; index--) {
			...
			BusinessPermitTile title = businessPermitTiles.remove(randomIndex); // Removes
																				// the
																				// tile
																				// at
																				// the
	...
	/**
	 * Changes the showed tiles with other tiles from the hidden deck. If you
	 * draw a showed card this method don't changes all showed cards,but shows a
	 * new card.
	 */
	public void changeShowedDeck() {
		if (showedDeck.size() == maxTilesShowed) // If the showed deck is full,
													// empties it.
			while (!showedDeck.isEmpty()) // Puts the showed tiles in the hidden
											// deck.
				hiddenDeck.add(showedDeck.remove(0));
		while (showedDeck.size() < maxTilesShowed && !hiddenDeck.isEmpty()) // Puts
																			// tiles
																			// in
																			// the
																			// showed

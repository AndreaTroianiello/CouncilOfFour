...
/**
 * The Region's class. The region is characterized by a name, a bonus and
 * contains a list of city, a council and a deck of tiles.
 * 
 * @author Andrea
 *
 */
public class Region implements Serializable {
	 ...
	 *            The region's name.
	 * @param points
	 *            The victory points which the bonus of region gives.
	 * @param deck
	 *            Region's deck. This deck contains business permit tiles.
	 * @param bonusKing
	 *            The bonus king.
	 */
	...
	/**
	 * Returns the status of the bonus region. If true, the bonus is already
	 * available.
	 * 
	...
	public List<City> searchCitiesByType(String type) {
		...
		for (City city : cities) // Explore the city.
			if (type.equals(city.getType())) // Control if the city has the same
												// type.
	 ...
	 * 
	 * @param player
	 * @return the status of the region. If true the region is completed by
	 *         player.
	 */
	public boolean completedRegion(Player player) {
		boolean allCities = true;
		for (City city : cities) // Explore the region's cities.
			allCities = allCities && city.containsEmporium(player); // Control
																	// if the
																	// city
																	// contains
																	// a
	...
	/**
	 * Runs the bonus associated to the region and sets the status of the bonus
	 * at false.
	 * 
	 * @param player
	 *            the player that takes the bonus of the region.
	 */
	public void runBonusRegion(Player player) {
		if (bonusAvailable && completedRegion(player)) { // Excute the bonus if
															// it's available
															// and the player
															// has emporiums in
															// all cities.
	...
	public String toString() {
		return "Region [name=" + name + ", bonus=" + bonus.getName() + ", cities=" + cities.size() + ", bonusAvailable="
				+ bonusAvailable + "]";
	}

...
/**
 * the class of the bonus that allows the player to step forward in the nobility
 * track.
 * 
 * @author Vincenzo
 *
...
public class BonusNobility implements Bonus {
	 ...
	 * 
	 * @param steps
	 *            the number of steps in the nobility track(can't be negative)
	 * @param board
	...
	public void giveBonus(Player player) {
		NobilityBox nobilityBoxNow = this.board.getNobilityTrack().getNobilityBoxes()
				.get(player.getNobilityBoxPosition());
		NobilityBox nobilityBoxNext = this.board.getNobilityTrack().getNobilityBoxes()
				.get(player.getNobilityBoxPosition() + steps);
		nobilityBoxNow.removePlayer(player);
		nobilityBoxNext.addPlayer(player);
		player.setNobilityBoxPoistion(player.getNobilityBoxPosition() + steps);
		for (Bonus b : nobilityBoxNext.getBonus()) {
			b.giveBonus(player);
		}
	}

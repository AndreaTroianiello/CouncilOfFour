...
/**
 * the class of the bonus that allows to run a bonus from a city where the
 * player builds an emporium.
 * 
 * @author Vincenzo
 *
 */
public class BonusCityToken extends Observable<Change>implements Bonus {
	...
	private List<City> cities; // the city the player chooses to run the bonus
								// from
	...
	/**
	 * the constructor set the parameter as the parameter given to the method,
	 * the city as null, and the name as CityToken
	 * 
	 * @param parameters
	 *            the number of cities the bonus can be run from
	 */
	...
	/**
	 * Control if the cities don't contains the nobility bonus and contain the
	 * player's emporium
	 * 
	 * @param realCities
	 *            the list of the cities
	 * @param player
	 *            who must have the emporiums in the cities
	 * @return false if the size of the list is different from number, and if a
	 *         city contains a nobility bonus or doesn't contain the player's
	 *         emporium, true otherwise
	 */
	private boolean controlParameters(List<City> realCities, Player player) {
		if (realCities.size() != number)
			return false;
		for (City city : realCities) {
			for (int i = 0; i < city.getToken().size(); i++) // iterate the
																// bonus in the
																// city
				if (city.getToken().get(i).getName().contains("BonusNobility") || !city.containsEmporium(player))
					return false;
		}
	 ...
	 * 
	 * @param realCities
	 *            the list where the real cities are added
	 */

...
/**
 * The class used for the election of the councillors. It contains only the controlAction.
 * 
 * @author Vincenzo
 *
 */
public class Elector implements Serializable{
	...
	public void election(Councillor councillor, Board board, Region region, boolean king){
		if(!king){		
			...
			board.setCouncillor(oldCouncillor);
			board.getKing().getCouncil().getCouncillors().add(councillor);							//append the chosen councillor in the same council

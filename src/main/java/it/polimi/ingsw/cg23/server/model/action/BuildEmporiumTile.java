...
/**
 * the class of the action that allows to build an emporium by using a
 * permitTile. I contains the BusinessPermitTile that the player chooses, the
 * city-id chosen in the tile and a boolean that show if it is a main action or
 * not.
 *
 * @author Vincenzo
 */
public class BuildEmporiumTile extends GameAction implements StandardAction {
	...
	private final City city; // which city the player choose from the ones on
								// the card
	private final ControlAction controlAction;

	/**
	 * the constructor set the variable of the class: the boolean main is set to
	 * true, the city and the cityId are set to the value of the parameter given
	 * 
	 * @param card
	 *            the tile used
	 * @param city
	 *            where to build the emporium
	...
	/**
	 * runAction controls if where is the chosen city and if the player has
	 * available emporiums, and if it has it builds the emporium in the chosen
	 * city
	 * 
	 * @param player
	 *            who runs the action

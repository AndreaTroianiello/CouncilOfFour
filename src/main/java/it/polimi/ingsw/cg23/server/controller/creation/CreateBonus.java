...
public class CreateBonus {
	...
	public void getCityBonus(City c) {
		...
		StringTokenizer st = new StringTokenizer(bonus());// string tokenizer
															// del nome dei
															// bonus
		while (st.hasMoreTokens()) {

			String name = st.nextToken(",");// estrae la sottostring fino alla
											// virgola
			b = name.substring(1, name.length());// isolo il nome del bonus
			int number = Integer.parseInt(name.substring(0, 1));// contiene il
																// numero es. 1
																// carta
																// politica, 2
																// coins

			for (int j = 0; j < bonusList.size(); j++) {// ciclo che scorre la
														// lista dei bonus
				if (bonusList.get(j).toString().contains(b)) {// controllo se il
					...
					Bonus bo = bonusList.get(j).copy();// clono il bonus preso
														// dalla lista dei bonus
	...
	private String bonus() {
		...
		int n = rnd.nextInt(bonus.length);// intero random massimo come il
											// numero di bonus
	 ...
	 *            the board
	 */
	public List<Bonus> bonusList(Board board) {// creo e aggiungo i bonus alla
												// lista bonus
	...
	public BonusKing bonusKing() {
		List<Integer> kingList = new ArrayList<>();// creo la lista con i bonus
													// del re
		kingList.add(25);
		kingList.add(18);
		kingList.add(12);
		kingList.add(7);
		kingList.add(3);
		kingList.add(0);// una lista di interi con i passi di avanzamento del
						// percorso vittoria
	 ...
	 * 
	 * @param bpt
	 *            the business permit tile
	 * @param bonusTotali
	 *            a string with all the bonuses
	 */
	public void getCostructorBonus(BusinessPermitTile bpt, String bonusTotali) {

		String b;// contiene il nome del bonus
		StringTokenizer st = new StringTokenizer(bonusTotali);// string
																// tokenizer del
																// nome dei
																// bonus
		...
		while (st.hasMoreTokens()) {// ciclo finche' ci sono token
			...
			for (int j = 0; j < bonusList.size(); j++) {// scorro la lista dei
														// bonus
				if (bonusList.get(j).toString().contains(b)) {// cerco il bonus
																// nella lista
																// dei bonus
					Bonus bo = bonusList.get(j).copy();
					bpt.addBonus(bo);// aggiungo alla carta costruzione i suoi
										// bonus

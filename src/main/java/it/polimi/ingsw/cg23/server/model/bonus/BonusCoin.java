...
public class BonusCoin implements Bonus {
	...
	/**
	 * the constructor set the coins as the parameter given to the method and
	 * the name as the name of the bonus
	 * 
	 * @param coin
	 *            (can't be negative)
	 */
	...
	public void giveBonus(Player player) {
		int playerCoins = player.getRichness().getCoins(); // set a variable at
															// the amount of
															// coins of the
															// player
		playerCoins = playerCoins + this.coins; // add to the variable the coins
												// given by the bonus
		try {
			player.getRichness().setCoins(playerCoins); // set the player's
														// coins at the updated
														// value and throw an
														// exception

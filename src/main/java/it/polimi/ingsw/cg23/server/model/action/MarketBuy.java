...
public class MarketBuy extends GameAction implements MarketAction {
	...
	/**
	 * Compares the assistants with the reference assistants. The item for sale
	 * must be a assistants pool.
	 * 
	 * @param itemToCompare
	 *            The assistants pool to compare.
	...
	/**
	 * Compares the politic card with the reference card. The item for sale must
	 * be a politic card.
	 * 
	 * @param itemToCompare
	 *            The politic card to compare.
	 * @return If true the business permit tile is the same.
	 */
	private boolean comparePoliticCard(PoliticCard itemToCompare) {
		...
		if (itemToCompare.getColor() != null) {
			if (itemToCompare.getColor().equals(refenceItem.getColor()))
				return true;
		} else if (itemToCompare.isJolly() == refenceItem.isJolly())
			return true;
		return false;
	}

	/**
	 * Compares the business permit tile with the reference tile. The item for
	 * sale must be a business permit tile.
	 * 
	 * @param itemToCompare
	 *            The business permit tile to compare.
	 * @return If true the business permit tile is the same.
	 */
	...
	/**
	 * Compares the item with the reference item. This method selected according
	 * to the type of item for sale.
	 * 
	 * @param itemToCompare
	 *            The item to compare.
	 ...
	 * @param board
	 *            the game's board.
	 * @return the item for sale from the market's list. If the item wasn't
	 *         found returns null.
	 */
	 ...
	 *             Throws if the player has negative number of assistants.
	 */

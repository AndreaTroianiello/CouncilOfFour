...
public class Rank{
	...
	/**
	 * Updates the victory points of the player whom has built the last emporium.
	 */
	...
	/**
	 * Updates the victory points of the players you create a rank by ordering list.
	 */
		...
		for(int index = 0; index < players.size(); index++) {
			...
			for(int j = 0; j < players.size()-1; j++) {
				...
				if(players.get(j).getVictoryTrack().getVictoryPoints()<players.get(j+1).getVictoryTrack().getVictoryPoints()) {
					...
					flag=true; //I put it to true to indicate that an exchange has taken place.
				}
				else
					//If they have the same score I look assistants and politic cards.
					if(players.get(j).getVictoryTrack().getVictoryPoints()==players.get(j+1).getVictoryTrack().getVictoryPoints()
			...
			if(!flag)
				return; //If flag = false then it means that last iteration there were no trade, then the method can terminate.
		}

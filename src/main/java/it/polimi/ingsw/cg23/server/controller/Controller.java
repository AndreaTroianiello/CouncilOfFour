...
public class Controller implements Observer<Action> {
	...
	/**
	 * The constructor of the Controller. Initializes the turn at null and the
	 ...
	 * 
	 * @param index
	 *            the number of the clients connected.
	...
	public void startGame() {
		...
		setPlayerStats();
		gameTwoPlayers();
		new CreateMap().createMapDraw(model); // Print the map on Server's
												// console.
	...
	private void gameTwoPlayers() {
		...
		if (model.getPlayers().size() == 2) {
			...
			for (Region r : reg) {
				List<Character> list = r.getDeck().getShowedDeck().get(rnd.nextInt(r.getDeck().getShowedDeck().size()))
						.getCitiesId();
				for (char c : list) {
	...
	public synchronized void update(Action action) {
		...
		if (interconnections.get(action.getPlayer()) == turn.getCurrentPlayer() && action instanceof GameAction) {

...
public class CityPanel{
	...
	public CityPanel(JTextArea loggerArea, ControllerGUI controller) {
		lung=Toolkit.getDefaultToolkit().getScreenSize().width-10.0;//larghezza schermo -10
	...
	public JPanel createCity(City c, King k){
		...
		//----------nome citta'----------
		JLabel nameLabel=new JLabel(Character.toString(c.getId()));//label nome della citta'
		double font= ((double) 2/100)*lung;//grandezza font lettera della citta'
		nameLabel.setFont(new Font("Calibre", Font.ITALIC,(int) font));//applicazione del font alla label
		...
		lim.gridwidth=1;
		layout.setConstraints(nameLabel, lim);//applicazione del layout alla label
		...
		if(c.equals(k.getCity())){//controllo se e' la citta' del re
			...
			JLabel kingLabel=new JLabel(new ImageIcon(imgKing));//creazione di una nuova label con il simbolo del re
			lim.gridx = p;//posizione componenti nella griglia
			lim.gridy = 0;
			lim.weightx = 1;//occupa tutto lo spazio all'interno del riquadro
			lim.weighty = 1;
			...
			if(c.getToken().isEmpty())//se la citta' non ha bonus incremento la posizione
				p++;

			layout.setConstraints(kingLabel, lim);//applicazione del layout alla label
		...
		if(!c.getToken().isEmpty()){//controllo se la citta' ha il bonus
			...
			double height=  ((double) imgCityBonus.getHeight()/imgCityBonus.getWidth())*width;//altezza immagine
			Image myim=imgCityBonus.getScaledInstance((int) width, (int) height, Image.SCALE_DEFAULT);//ridimensionamento immagine

			JLabel bonusCityLabel=new JLabel(new ImageIcon(myim));//creazione della label con il bonus
			lim.gridx = p;//posizione componenti nella griglia
			lim.gridy = 1;
			lim.weightx = 1;//occupa tutto lo spazio all'interno del riquadro
			lim.weighty = 1;
			...
			layout.setConstraints(bonusCityLabel, lim);//applicazioen layout alla label
		...
		double height=  ((double) img.getHeight()/img.getWidth())*width;//altezza immagine citta0
		Image myim=img.getScaledInstance((int) width, (int) height, Image.SCALE_DEFAULT);//ridimensionamento immagine

		JLabel label=new JLabel(new ImageIcon(myim));//creazione etichetta con l'immagine della citta'
		lim.gridx =p;//posizione componenti nella griglia
		lim.gridy = 0;
		lim.weightx = 1;//occupa tutto lo spazio all'interno del riquadro
		lim.weighty = 1;
	 ...
	 * @param panel, the panel to add the listener
	 * @param c, the actual city in the panel
	 * @param k, the king
	 */

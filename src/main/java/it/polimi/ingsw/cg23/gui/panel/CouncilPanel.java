...
public class CouncilPanel extends JPanel {
	...
	public JPanel balcone(Region reg){
		...
		for(int i=0; i<reg.getCouncil().getCouncillors().size(); i++){
			...
			JLabel label1 = new JLabel();//aggiungo l'immagine alla label
			label1.setBackground(reg.getCouncil().getCouncillors().get(i).getColor());
			//label1.se
			label1.setName("consigliere 1");
			label1.setPreferredSize(new Dimension(50, 50));
			//label1.setBounds(0, 0, 100, 100);//dimensioni della label
			lim.gridx = 0;//posizione componenti nella griglia
			lim.gridy = i;
			lim.ipadx=0;//bordi componente
			lim.ipady=0;
		...
		label1.setName("consigliere 1");
		//label1.setBounds(0, 0, 10, 10);//dimensioni della label
...
/*
		...
		JLabel label2 = new JLabel();//aggiungo l'immagine alla label
		label2.setBackground(reg.getCouncil().getCouncillors().get(1).getColor());
		label2.setName("consigliere 2");
		label2.setBounds(0, 0, 10, 10);//dimensioni della label

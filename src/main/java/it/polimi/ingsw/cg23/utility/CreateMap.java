...
public class CreateMap {
	 ...
	 */
	public String createMap(List<Region> reg, List<Player> giocatori, King king) {// NON
																					// TIENE
																					// CONTO
																					// DEI
																					// COLLEGAMENTI
		...
		gamemap += ms.printName(reg, space) + "\n";// aggiungo i nomi delle
													// regioni

		int regionNumber = reg.size();// recupera il numero di regioni
		for (int i = 0; i < city.size() / regionNumber; i++) {// ciclo che
																// scorre le
																// citta' per
																// regione da
																// stampare 5

			for (int k = 0; k < regionNumber; k++) {// aggiunge le 3 citta', una
													// per ogni regione
				int kk = i + (city.size() / regionNumber) * k;// salvo il valore
																// di k

				String newcity = city.get(kk).getName() + "(" + city.get(kk).getType() + ")"
						+ city.get(kk).getEmporiums() + "";// recupero le
															// informazioni
															// dall'array

				String citybonus = ms.cityBonus(city.get(kk));// bonus della
																// citta'

				newcity += "(" + citybonus + ")";// aggiungo alla nuova citta' i
													// suoi bonus
				String kingCity = king.getCity().getName();// nome della citta'
															// del re

				if (kingCity.equals(city.get(kk).getName())) {// se la città e'
																// quella che
																// sta ciclando
																// la segno
					newcity = newcity.substring(0, newcity.length() - 2) + "KING";// la
																					// citta
																					// del
																					// re
																					// non
					...
					gamemap = gamemap.concat(ms.addSpace(newcity, space));// aggiungo
																			// la
																			// citta'
																			// appena
																			// aggiunta
			...
			gamemap = gamemap.concat("\n");// aggiungo un a capo dopo aver messo
											// 3 citta' su una riga (una per
											// regione)
		}

		gamemap = gamemap.concat("\n");
		gamemap = gamemap.concat(createPlayerInfo(giocatori));// aggiunge alla
																// plancia di
																// gioco i
																// punteggi
																// giocatore
	...
	public String createMapDraw(Board b) {
		...
		List<City> city = ms.getCityfromRegion(reg);
		String plancia = "\nGame Board\n";// la stringa che stampa la plancia di
											// gioco
		...
		for (int i = 0; i < citypReg; i++) {// ciclo che scorre le citta' per
											// regione da stampare 5
			int minus = 32;
			...
			for (int k = 0; k < regionNumber; k++) {// ciclo che aggiunge la il
													// nome della citta'
				String name;
				if (b.getKing().getCity().getName().equals(city.get(i + citypReg * k).getName())) {
					name = ms.addSpace(
							"|" + city.get(i + citypReg * k).getName() + " - " + city.get(i + citypReg * k).getId(),
							minus - 1) + "|";
					name = name.substring(0, name.length() - 7);// faccio spazio
																// per
																// aggiungere
																// king
				...
				plancia = plancia.concat(ms.addSpace(name, space));// aggiungo
																	// il nome
																	// della
																	// citta'
			}
			...
			for (int k = 0; k < regionNumber; k++) {// ciclo che aggiunge il
													// tipo(colore) della citta'
				String tipo = ms.addSpace("|" + city.get(i + citypReg * k).getType(), minus - 1);
				tipo = tipo.concat("|");
				plancia = plancia.concat(ms.addSpace(tipo, space));// aggiungo
																	// il tipo
																	// della
																	// citta'
			}
			...
			for (int k = 0; k < regionNumber; k++) {// ciclo che aggiunge i
				...
				plancia = plancia.concat(ms.addSpace(bonus, space));// aggiungo
																	// i bonus
																	// della
																	// citta'
			}
			...
			for (int k = 0; k < regionNumber; k++) {// ciclo che aggiunge i
				...
				plancia = plancia.concat(ms.addSpace(vicini, space));// aggiungo
																		// i
																		// vicini
																		// della
																		// citta'
			...
			for (int k = 0; k < regionNumber; k++) {// ciclo che aggiunge gli
													// empori
				String empori = ms.addSpace("|Emporiums:" + city.get(i + citypReg * k).getEmporiums(), minus - 1) + "|";
				plancia = plancia.concat(ms.addSpace(empori, space));// aggiungo
																		// gli
																		// empori
																		// della
																		// citta'
		...
		plancia = plancia.concat("\n");
		plancia += ms.createCostructionShowed(reg, space / 2);// aggiungo le
																// carte
																// costruzione
																// alla plancia
		...
		plancia += createPlayerInfo(b.getPlayers());// aggiunge alla plancia di
													// gioco i punteggi
													// giocatore
	...
	public String printNobility(Board b) {
		...
		for (int i = 0; i < nb.size(); i++) {// aggiunge il numero di box e il
												// primo bonus (se c'è)
			if (nb.get(i).getBonus().isEmpty())
				nobility = nobility.concat(ms.addSpace(i, nobilitySpace - 2) + "|");
			else
				nobility = nobility
						.concat(ms.addSpace(i + " " + nb.get(i).getBonus().get(0).getName(), nobilitySpace - 2) + "|");
		}

		nobility += "\n|";

		for (int i = 0; i < nb.size(); i++) {// aggiunge il secondo bonus (se
												// c'è)
		...
		for (int i = 0; i < nb.size(); i++) {// aggiunge i giocatori (se ci
												// sono)
			String giocatori = "";
			...
			if (giocatori.length() == 0)
				...
				nobility = nobility
						.concat(ms.addSpace(giocatori.substring(0, giocatori.length() - 2), nobilitySpace - 2) + "|");
		}
	...
	public String createPlayerInfo(List<Player> giocatori) {
		...
		for (int i = 0; i < giocatori.size(); i++) {// stampa i punteggi dei
													// giocatori
			percorsi = percorsi.concat(ms.addSpace(giocatori.get(i).getUser(), playerSpace));// aggiungo
																								// il
																								// nome
																								// dei
																								// giocatori
			percorsi = percorsi.concat(ms.addSpace(giocatori.get(i).getRichness().getCoins(), playerSpace));// aggiungo
																											// il
																											// numero
																											// di
																											// coins
			percorsi = percorsi.concat(ms.addSpace(giocatori.get(i).getVictoryTrack().getVictoryPoints(), playerSpace));// aggiungo
																														// i
																														// victory
																														// points
			percorsi = percorsi.concat(ms.addSpace(giocatori.get(i).getNobilityBoxPosition(), playerSpace));// aggiungo
																											// la
																											// posizione
																											// sul
																											// nobility
																											// box
			percorsi = percorsi.concat(ms.addSpace(giocatori.get(i).getAssistantsPool().getAssistants(), playerSpace));// aggiungo
																														// il
																														// numero
																														// di
																														// assistenti
			percorsi = percorsi.concat(ms.addSpace(giocatori.get(i).getHand().size(), playerSpace));// aggiungo
																									// il
																									// numero
																									// di
																									// carte

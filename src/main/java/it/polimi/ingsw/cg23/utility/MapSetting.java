...
public class MapSetting {
	 ...
	 * 
	 * @param city
	 *            the city you want to have the bonus
	 * @return a string with all the cities' bonus
	 */
	public String cityBonus(City city) {
		String bonus = "";

		for (int i = 0; i < city.getToken().size(); i++) {// ciclo che scorre i
															// bonus di una
															// citta'
			bonus = bonus.concat(city.getToken().get(i).getName());// ritorna il
																	// nome del
																	// bonus
			bonus = bonus.concat(", ");
		}
		if (bonus.length() == 0) // se non ci sono bonus ritorna una stringa
									// vuota
			return "";
		bonus = bonus.substring(0, bonus.length() - 2);// tolgo l'ultima virgola
														// e spazio
		return bonus;
	}
	 ...
	 * 
	 * @param nome
	 *            the String you want to extend
	 * @param totalSpace
	 *            the total length you need (string + space)
	 * @return the String extended
	 */
	public String addSpace(String nome, int totalSpace) {// aggiunge spazi alla
															// stringa data per
															// raggiungere la
															// lunghezza
															// desiderata
	...
	/**
	 * @param number
	 *            the number to add space
	 * @param totalSpace
	 *            the number of space you want to add
	 ...
	 * 
	 * @param number
	 *            the number you want to calculate the digits
	...
	public String createCostructionShowed(List<Region> region, int space) {
		...
		for (int j = 0; j < regSize; j++) {// ciclo che scorre le regioni
			cardShowed = cardShowed
					.concat(addSpace("Business Permit Tiles " + region.get(j).getName() + ":", space * 2));// carte
																											// costruzione
		}
		...
		for (int j = 0; j < regSize; j++) {// ciclo che scorre le regioni
			String mino = "";
			List<BusinessPermitTile> costruction = region.get(j).getDeck().getShowedDeck();// recupero
																							// la
																							// lista
																							// delle
																							// carte
			...
			for (int i = 0; i < costruction.size(); i++) {// ciclo che scorre le
															// carte costruzione
															// della regione
				mino = mino.concat(addSpace(addMinus(space - 5), space));// aggiungo
																			// i
																			// -
			}
			cardShowed = cardShowed.concat(mino);// aggiungo il tutto alla
													// string finale
		}
		cardShowed = cardShowed.concat("\n");
		...
		for (int j = 0; j < regSize; j++) {// ciclo che scorre le regioni
			...
			for (int i = 0; i < costruction.size(); i++) {// ciclo che scorre le
				...
				card += addSpace("|" + costruction.get(i).getCitiesId().toString(), space - 6);// aggiungo
																								// gli
																								// id
																								// delle
																								// citta'
				...
				cardShowed = cardShowed.concat(card);// aggiungo il tutto alla
														// string finale
			}
		}
		cardShowed = cardShowed.concat("\n");
	...
	private String getBonusCostructor(List<Region> reg, int space, int n) {
		...
		for (int j = 0; j < reg.size(); j++) {// ciclo che scorre le regioni
			List<BusinessPermitTile> costruction = reg.get(j).getDeck().getShowedDeck();// recupero
																						// la
																						// lista
																						// delle
																						// carte
			...
			for (int i = 0; i < costruction.size(); i++) {// ciclo che scorre le
				...
				List<Bonus> bo = costruction.get(i).getBonusTile();// lista con
																	// i bonus

				if (bo.size() > 1 || n < 1) {// il secondo bonus può non esserci
					bon = bon.concat(addSpace("|" + bo.get(n).getName(), space - 6));
				} else {
					bon = bon.concat(addSpace("|", space - 6));// nel caso non
																// ci sia il
																// secondo bonus
				}
				bon = addSpace(bon + "|", space);// aggiungo gli spazi
				bonusName = bonusName.concat(bon);// aggiungo il tutto alla
													// string finale
			}
		}
	...
	public String getNeighbourID(City c) {
		List<City> vicini = c.getNeighbors();// recupero la lista dei vicini
												// della citta'
		String viciniId = "Neighbors: ";// stringa che contiene i vicini della
										// citta'

		for (int i = 0; i < vicini.size(); i++) {// scorre il numero di vicini
													// della citta'
			viciniId = viciniId.concat(Character.toString(vicini.get(i).getId()));// aggiunge
																					// alla
																					// stringa
																					// l'id
																					// della
		...
		return viciniId.substring(0, viciniId.length() - 2);// tolgo gli ultimi
															// due caratteri
															// (virgola e
															// spazio)
	}
	 ...
	 * 
	 * @param number
	 *            the number of minus you want
	 * @return a string with minus
	 */
	public String addMinus(int number) {
		String minus = "";

		for (int i = 0; i < number; i++) {// ciclo che scorre la quantita' di
											// meno da aggiungere
	...
	public String bonusCouncilKingType(Board b) {
		...
		List<Councillor> kingCouncillors = b.getKing().getCouncil().getCouncillors();// consiglieri
																						// del
																						// re

		for (int i = 0; i < kingCouncillors.size(); i++) {
			consiglieri = consiglieri.concat(cm.getColorName(kingCouncillors.get(i).getColor()));// converto
																									// i
																									// colori
																									// in
																									// nomi
		...
		for (int i = bk.getCurrentIndexBonusKing(); i < bk.getBonusValues().size() - 1; i++) {// scorre
																								// i
																								// bonus
																								// king
		...
		for (int i = 0; i < tipes.size(); i++) {
			if ("Purple".equals(tipes.get(i).getName()))
				tipi = tipi.concat("");
			else if (tipes.get(i).isBonusAvailable())
				tipi = tipi.concat(tipes.get(i).getName() + ": " + tipes.get(i).getBonus().getNumber() + ", ");
		}

		return addSpace(consiglieri, space) + addSpace(bonus, space)
				+ addSpace(tipi.substring(0, tipi.length() - 2), space);
	}
